...
class F110Dataset(torch.utils.data.Dataset):
    def __init__(self, scope, folder, labels, images, label_arr, input_dim, anchor_size, final_dim):
        self.folder = folder
        self.labels = labels
    ...
    def __getitem__(self, index):
        label = self.labels[self.label_arr[index]].copy() / (360.0 / self.input_dim[0])
        label_gt = np.asarray(assign_label(label, self.input_dim, self.anchor_size, self.final_dim))
        img_np = self.images[self.label_arr[index]]
...
def assign_label(label, input_dim, anchor_size, final_dim):
    ...
    grid_info = grid_cell(IoU_max_ind[0], IoU_max_ind[1], anchor_size)
...
def label_to_box_xyxy(result, input_dim, final_dim, anchor_size, threshold = 0.9):
    validation_result = []
    result_prob = []
    for ind_row in range(final_dim[0]):
        for ind_col in range(final_dim[1]):
